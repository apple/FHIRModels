 ...
 */
public enum SubscriptionErrorCodes: String, FHIRPrimitiveType {
	
	/// DNS resolution error for client rest-hook endpoint.
	case dnsResolutionError = "dns-resolution-error"
	
	/// No response for client rest-hook endpoint.
	case noResponse = "no-response"
	
	/// Error response from client rest-hook endpoint. Recommended practice: convey the actual HTTP error in an adjacent
	/// Coding or in text
